#!/usr/bin/env python3
"""
make_8_mathematical_context_for_rk.py

Record the external mathematical context for r_k and the psi_k row family.

Output:
    8_mathematical_context_for_rk.md
"""
...
x, k, m = sp.symbols("x k m", positive=True, integer=True)
a = 1 - x**2
...
# Beta ratio behind the primitive/moment family.
beta_ratio = simplify_expr((k - sp.Rational(1, 2)) / (k + m - sp.Rational(1, 2)))
expected_ratio = (2 * k - 1) / (2 * k + 2 * m - 1)
require_zero("beta ratio", beta_ratio - expected_ratio)
checks.append("beta ratio r_(k,m)")

# Pearson/divergence identity for auxiliary weight rho=a^2.
rho = a**2
sigma = a
tau = simplify_expr(sp.diff(sigma * rho, x) / rho)
require_zero("Pearson tau for rho=a^2", tau + 6 * x)
checks.append("Pearson equation for auxiliary Gegenbauer weight a^2")

f = sp.Function("f")(x)
L_f = sigma * sp.diff(f, x) + tau * f
divergence_form = simplify_expr(sp.diff(sigma * rho * f, x) / rho)
require_zero("Pearson divergence operator", L_f - divergence_form)
checks.append("L as Pearson divergence operator")

# Primitive identity for observed m=2.
r_k = (2 * k - 1) / (2 * k + 3)
psi_k = x ** (2 * k) - r_k * x ** (2 * k - 2)
primitive = x ** (2 * k - 1) * a**2
require_zero("primitive identity", sp.diff(primitive, x) + (2 * k + 3) * a * psi_k)
checks.append("primitive identity for observed ratio")

validation_bullets = "\n".join("- " + check + ": passed" for check in checks)

md = f"""# Field Search Survivor Audit 8: Mathematical Context for r_k
...
out = Path(__file__).with_name("8_mathematical_context_for_rk.md")
out.write_text(md, encoding="utf-8")

print("Mathematical context checks passed.")
