...
| creation seam decided | 054 | EXPANSION IS CREATION, decided on existing data: the stretch reading killed twice (floor-Newton lock ⟹ Ġ/G = −H, excluded ~480× by LLR; dilation-invariant wedges ⟹ floor turns to unsequestered Planck-density dust, ~10¹²²× closure). The growth reading survives and is exactly self-funding: 051's measured intensivity IS w = −1 at the substance level, and creation's cost u·dV equals the floor's negative-pressure work term by term. NEW register entry A6: Ġ = 0 exactly (under real pressure — H₀ only ~480× above LLR bounds). Vacuum-flow clause formalized (C = 3H(1−αδ), background-invisible); α awaits O-P10-4. |
| boundary channel kill | 053 | the owed Casimir/UFFT operator instantiation delivered — and it KILLS the unsuppressed channel: the packing's boundary operator is a constant attractive pressure (form distinct from Casimir d⁻⁴) whose floor-Newton-locked magnitude (5×10¹¹² Pa at Planck a; 1.5×10⁵² even at window-scale a) is excluded by 54–114 orders. Matter boundaries cannot confine the packing; the floor hides from BOTH channels; the UFFT window loses its VED motivation (B3 downgraded); the engineered-boundary release mechanism closes. Cycle nets exactly zero (Hartle–Sorkin additivity) — died of magnitude, not thermodynamics. |
| edge density | 052 | O-P10-1 partial: c_e DERIVED — the 3D flat ground mixture (x_6 = 2π/arccos(1/3) − 5 = 0.1043) fixes mean edge coordination 2π/arccos(1/3) = 5.1043, giving c_e = 36 arccos(1/3)/(√2 π) = 9.9743 per a³, pure geometry (bulk cross-check: coordination within 10%). With f'(Δ₀) locked by 039, ρ_v = 0.6403 f'(Δ₀)/a³ — ONE unknown left (the packing scale a). C4 sharpens into the two-face discreteness battery (floor-Newton lock + √6·a scalaron). |
| bulk relaxation phase 2 | 051 | O-P10-5 phase 2 delivered (seeded, deterministic): floor INTENSIVE (E/N stable across ×4 in size — a bulk density, 038's requirement); first 4D realizability data (hinge mean 4.98 → 4.86 under smoothing, toward 050's 4.7668; hinge identity < 1%); defect spectrum measured (positive, quantized, deficit-ordered); disclination NETWORKS observed (defect edges form one spanning component). Opened: periodic_energy_relaxation_051 (phase 3 — the full realizability test). Dark-excess gates unchanged. |
| 4D ground coordination | 050 | O-P10-2 resolved at mean-field: the flat 4D ground state is a FORCED {4, 5} MIXTURE — n = 5 is the softest wedge (exact ladder + 4D relaxation lab, E(5) = 0.0048 strict minimum) but pure n = 5 is negatively curved; zero mean deficit fixes x_4 = 5 − 2π/arccos(1/4) = 0.2332, mean coordination 4.7668, exactly. Mixed floor strictly positive: flatness is bought with frustration energy (038's identity, sharpest form). Opened: mixture_realizability_050 (joint with O-P10-5). |
