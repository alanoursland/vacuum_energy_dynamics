...
| trace normalization / \(P_{\rm trace\_norm}\) | `COMPATIBLE_IF_DECLARED` / `CANDIDATE_REMAINS` / `PAIRED_SCOPE_STATUS_RECORD` / `READY_FOR_DECLARATION_ATTEMPT_WITH_CONDITIONS` / `NOT_DECLARED` | Candidate forms for how \(B_s\) reads \(\zeta\) | Metric and scale trace-normalization records are explicit and non-active; \(\zeta\) and symbolic \(d\) are review-closed; the paired declaration-scope/status record is accepted as input; symbolic \(d\) may be carried into a separate paired declaration attempt; numeric \(d\) remains conditioned and unfixed; next target is a separate symbolic paired trace-normalization declaration attempt; no branch selected, no declaration completed, no insertion licensed |
...
Next honest moves:
  separate symbolic paired trace-normalization declaration attempt with explicit caveats and failure criteria,
