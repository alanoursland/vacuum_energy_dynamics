...
| trace normalization / \(P_{\rm trace\_norm}\) | `COMPATIBLE_IF_DECLARED` / `CANDIDATE_REMAINS` / `PARALLEL_RECORD_SURFACE_EXPLICIT` / `REVIEW_READY_ONLY` / `PAIRED_SCOPE_STATUS_RECORD` / `NOT_DECLARED` | Candidate forms for how \(B_s\) reads \(\zeta\) | Metric and scale trace-normalization records are explicit and non-active; \(\zeta\) and symbolic \(d\) are closed for paired record review; the explicit paired declaration-scope/status record is now instantiated; it carries identity, paired-record domain, status, assumptions, numeric-\(d\) condition, branch status, caveats, and handoff route; no branch selected, no declaration completed, no insertion licensed |
...
Trace-anchor choice surface:
  ...
  Scope-record-ready is not declaration-ready.
...
Next honest moves:
  declaration-readiness review for a possible parallel trace-normalization declaration attempt,
  separate parallel trace-normalization declaration record only if declaration-readiness review accepts the assumptions and caveats,
