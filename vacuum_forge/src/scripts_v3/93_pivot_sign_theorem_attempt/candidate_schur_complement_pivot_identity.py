...
DEPENDENCIES = [('g93_row_sign_matrix', '93_pivot_sign_theorem_attempt__candidate_row_sign_normalized_matrix', 'g93_row_sign_matrix')]
...
def main():
    ...
    failures=[]; rows=[]; prev=sp.Integer(1)
    for N in range(1,16):
        B=row_signed_matrix(N); detB=sp.factor(B.det(method='bareiss')); pivot=sp.factor(detB/prev)
        if N==1: schur=sp.factor(B[0,0])
        else:
            C=B[:N-1,:N-1]; u=B[:N-1,N-1]; v=B[N-1,:N-1]; alpha=B[N-1,N-1]
            schur=sp.factor(alpha-(v.T*C.LUsolve(u))[0])
        diff=sp.factor(schur-pivot)
        if diff!=0: failures.append((N,diff))
        rows.append((N,schur,sp.sign(schur))); prev=detB
    header('Candidate Schur Complement Pivot Identity')
    print('Schur/pivot failures through N=15:', failures)
    with out.derived_results():
        out.line('Schur failures', StatusMark.PASS if not failures else StatusMark.FAIL, str(failures))
        for N,schur,sgn in rows: out.line(f'N={N} Schur sign', StatusMark.PASS if sgn>0 else StatusMark.FAIL, str(sgn))
    ...
    ns.record_derivation(derivation_id=MARKER_ID, inputs=[], output=sp.Matrix([r[2] for r in rows]), method='verify pivots equal Schur complements through N=15', status=Status.DERIVED, record_kind=RecordKind.DERIVATION, result_type='schur_complement_pivot_identity', scope='structural pivot theorem')
    record_claim(ns, MARKER_ID, 'g93_schur_c1', GovernanceStatus.POLICY_RULE, 'Sign-normalized pivot positivity is equivalent to positive leading Schur complements.')
    record_obligation(ns, 'g93_schur_o1', 'Test total positivity route.')
