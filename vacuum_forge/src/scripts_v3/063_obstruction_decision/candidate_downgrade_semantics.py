...
SCOPE = 'Group 63 downgrade semantics'
NEXT_SCRIPT = 'candidate_retention_contract.py'

ENTRIES = [('G1: preserved material', 'residues, N_w, eta/eta^2, P obstruction, I_P=2E_pr', 'DIAGNOSTIC_ONLY_STATUS_DEFINED', 'diagnostics remain useful', 'nonphysical'), ('G2: forbidden upgrades', 'term/stress/source/mass/trace/parent use', 'DIAGNOSTIC_ONLY_STATUS_DEFINED', 'diagnostic-only status blocks physical interpretation', 'hard boundary'), ('G3: downgrade allowed', 'candidate may be downgraded without losing lessons', 'DIAGNOSTIC_DOWNGRADE_ALLOWED', 'clean downgrade is progress', 'allowed')]
SHORTCUTS = [('X1: downgrade as deletion', 'discard all boundary-layer findings', 'diagnostics remain valuable'), ('X2: diagnostic as candidate', 'use diagnostic-only object as term', 'status forbids physical use'), ('X3: diagnostic as source', 'use diagnostic-only object as mass/source response', 'not licensed')]
OBLIGATIONS = [('O1: snapshot clarity', 'POLICY_RULE', 'snapshot must distinguish diagnostic preservation from candidate permission', 'avoid false failure/false success')]
LOCAL_CONCLUSIONS = [('downgrade semantics defined', 'DIAGNOSTIC_ONLY_STATUS_DEFINED', 'diagnostic-only status preserves lessons but forbids physical use'), ('diagnostic downgrade allowed', 'DIAGNOSTIC_DOWNGRADE_ALLOWED', 'downgrade remains a clean option')]
...
def case_downgrade_semantics(out: ScriptOutput):
    header("Case 0: Diagnostic-only downgrade semantics")
    ...
    with out.derived_results():
        out.line("preserved count", StatusMark.PASS, str(len(preserved)))
        out.line("forbidden count", StatusMark.PASS, str(len(forbidden)))

    return {"preserved_count": sp.Integer(len(preserved)), "forbidden_count": sp.Integer(len(forbidden))}


def record_down(ns, data) -> None:
    ns.record_derivation(
        derivation_id="g63_down",
        inputs=[],
        output=data["forbidden_count"],
        method="define diagnostic-only downgrade semantics",
        status=Status.DERIVED,
        record_kind=RecordKind.INVENTORY_MARKER,
        result_type="diagnostic_only_semantics",
        scope="status semantics; no physical claim",
    )
...
def main() -> None:
    ...
    data = case_downgrade_semantics(out)
