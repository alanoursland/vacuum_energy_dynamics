...
def main():
    ...
    B=row_signed_matrix(12); neg=[]
    for i in range(12):
        for j in range(12):
            if B[i,j]<0: neg.append((i+1,j+1,B[i,j]))
    header('Candidate Total Positivity Obstruction')
    print('negative 1x1 entries in B_12:', len(neg)); print('first negatives:', neg[:10])
    with out.derived_results(): out.line('negative entry count', StatusMark.WARN, str(len(neg)))
    with out.governance_assessments():
        out.line('total positivity route', StatusMark.WARN, 'blocked by negative 1x1 entries')
        out.line('leading minor route', StatusMark.INFO, 'leading principal positivity remains possible')
    record_marker(ns, MARKER_ID, 'total positivity obstruction')
    record_claim(ns, MARKER_ID, 'g93_tp_c1', GovernanceStatus.POLICY_RULE, 'Strict total positivity of B_N is blocked by negative entries.')
    record_obligation(ns, 'g93_tp_o1', 'Test principal minor route.')
