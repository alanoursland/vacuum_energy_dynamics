...
SCOPE = 'Group 64 functional requirements'
NEXT_SCRIPT = 'candidate_quadratic_scalar_origin.py'

ENTRIES = [('R1: profile/stress basis', 'derive eta or eta^2 role', 'REQUIREMENTS_RESTATED', 'profile cannot be merely chosen', 'required'), ('R2: closure', 'derive u relation', 'REQUIREMENTS_RESTATED', 'trace/mass accounting needs derived energy density', 'required'), ('R3: amplitude', 'derive p_free', 'REQUIREMENTS_RESTATED', 'source-independent amplitude remains underived', 'required'), ('R4: safety', 'source/trace/mass/covariance/boundary behavior', 'REQUIREMENTS_RESTATED', 'origin must not hide load', 'required')]
SHORTCUTS = [('X1: profile by taste', 'choose eta because it is convenient', 'must be derived or diagnostic-only'), ('X2: closure by taste', 'choose u=P or u=-P', 'only one diagnostic closes'), ('X3: amplitude by normalization', 'use scale as physics', 'normalization is not origin')]
OBLIGATIONS = [('O1: origin tests', 'ENERGY_ACCOUNTING_REQUIRED', 'test simple origins against these requirements', 'avoid decorative principle')]
LOCAL_CONCLUSIONS = [('requirements restated', 'PASS', 'origin route now has explicit pass conditions'), ('retention contract remains', 'RETENTION_CONTRACT_REQUIRED', 'candidate is not live without these burdens')]
...
def case_requirements(out: ScriptOutput):
    header("Case 0: Functional-origin requirement inventory")

    requirements = [
        "derive eta-like profile or stress basis",
        "derive u relation",
        "derive p_free scale",
        "preserve source independence",
        "preserve boundary value/slope silence",
        "address trace/mass status",
    ...
    with out.derived_results():
        for item in requirements:
            out.line(item, StatusMark.OBLIGATION, "required for real variational/stress origin")

    return {"requirement_count": sp.Integer(len(requirements))}


def record_req(ns, data) -> None:
    ns.record_derivation(
        derivation_id="g64_req",
        inputs=[],
        output=data["requirement_count"],
        method="inventory requirements for a real reduced variational/stress origin",
        status=Status.DERIVED,
        record_kind=RecordKind.INVENTORY_MARKER,
        result_type="functional_origin_requirements",
        scope="requirements inventory; no closure theorem",
    )
