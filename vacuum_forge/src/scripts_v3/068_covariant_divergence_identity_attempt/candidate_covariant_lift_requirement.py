...
SCOPE = 'Group 68 covariant lift requirement'
NEXT_SCRIPT = 'candidate_boundary_divergence_neutrality.py'

ENTRIES = [('L1: lift relation', 'D_cov = D_reduced + L_error', 'COVARIANT_LIFT_REQUIRED', 'reduced balance must be lifted with no lift error', 'required'), ('L2: exact lift condition', 'L_error=0', 'COVARIANT_LIFT_NOT_PROVEN', 'no covariant lift theorem is proved here', 'open'), ('L3: reduced balance', 'not a Bianchi identity', 'DIVERGENCE_IDENTITY_NOT_PROVEN', 'symbolic reduced balance is not parent conservation', 'open')]
SHORTCUTS = [('X1: reduced as covariant', 'treat reduced D balance as covariant identity', 'lift error not controlled'), ('X2: assume lift error zero', 'set L_error=0 by prose', 'requires theorem')]
OBLIGATIONS = [('O1: boundary neutrality', 'BOUNDARY_NEUTRALITY_REQUIRED', 'audit D_boundary requirement next', 'avoid hiding boundary burden in lift')]
LOCAL_CONCLUSIONS = [('covariant lift required', 'COVARIANT_LIFT_REQUIRED', 'D_lift cannot be ignored'), ('covariant identity not proven', 'DIVERGENCE_IDENTITY_NOT_PROVEN', 'lift theorem remains open')]
...
def case_covariant_lift(out: ScriptOutput):
    header("Case 0: Covariant lift requirement")
    ...
    with out.derived_results():
        out.line("lift relation", StatusMark.OBLIGATION, str(lift_relation))
        out.line("exact lift condition", StatusMark.OBLIGATION, f"L_error={lift_required}")

    return {"lift_condition": sp.Symbol("L_error_equals_zero")}


def record_lift(ns, data) -> None:
    ns.record_derivation(
        derivation_id="g68_lift",
        inputs=[],
        output=data["lift_condition"],
        method="record covariant lift condition required to turn reduced divergence balance into covariant identity",
        status=Status.DERIVED,
        record_kind=RecordKind.INVENTORY_MARKER,
        result_type="covariant_lift_requirement",
        scope="covariant lift requirement; no lift theorem",
    )
