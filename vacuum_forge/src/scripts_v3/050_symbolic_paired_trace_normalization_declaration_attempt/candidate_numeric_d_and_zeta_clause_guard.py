...
SCOPE = 'Group 50 numeric d and zeta clause guard'
NEXT_SCRIPT = 'candidate_declaration_attempt_failure_sieve.py'
LOCAL_CONCLUSIONS = [('zeta/d clauses guarded', 'PASS', 'symbolic and conditioned clauses remain safe for the attempt'), ('numeric d not fixed', 'NUMERIC_D_CONDITION', 'numeric dimension remains conditioned')]

def build_entries() -> List[Entry]:
    return [
        Entry('D1: zeta clause', 'zeta remains shared record-local trace-payload symbol', 'ZETA_CLAUSE', 'zeta can appear in both candidate expressions', 'not F_zeta or active field'),
        Entry('D2: symbolic d clause', 'd remains symbolic traced-dimension field', 'SYMBOLIC_D_ALLOWED', 'symbolic d may be carried by the attempt', 'not numeric declaration'),
        Entry('D3: numeric d condition', 'numeric d remains conditioned and unfixed', 'NUMERIC_D_CONDITION', 'future scope support would be needed to fix it', 'not closed here'),
        Entry('D4: parent-facing block', 'symbolic d and zeta are not parent-facing objects', 'THEOREM_REQUIRED', 'parent-facing use requires identity and safety support', 'not available'),
    ]

def build_shortcuts() -> List[Shortcut]:
    return [
        Shortcut('X1: numeric d leak', 'quietly set d=3 or another number', 'numeric d remains conditioned'),
        Shortcut('X2: d by recovery', 'choose d from Schwarzschild, AB=1, gamma, or weak-field recovery', 'recovery is audit only'),
        Shortcut('X3: zeta as response', 'treat zeta as F_zeta or insertion response', 'zeta payload and response placeholder remain separate'),
        Shortcut('X4: parent d', 'treat symbolic d as parent dimension', 'parent-facing support absent'),
    ]

def build_obligations() -> List[ObligationEntry]:
    return [
        ObligationEntry('O1: preserve numeric-d condition', 'NUMERIC_D_CONDITION', 'state that numeric d is not fixed by the attempt', 'avoid hidden numeric postulate'),
        ObligationEntry('O2: preserve zeta role', 'POLICY_RULE', 'keep zeta as trace payload only', 'avoid response-map drift'),
        ObligationEntry('O3: preserve parent block', 'THEOREM_REQUIRED', 'do not promote clauses to parent-facing use', 'avoid parent drift'),
    ]
