...
SCOPE = 'Group 50 symbolic paired declaration attempt problem'
NEXT_SCRIPT = 'candidate_paired_declaration_attempt_record.py'
LOCAL_CONCLUSIONS = [('Group 50 opener complete', 'PASS', 'symbolic paired declaration attempt opened'), ('no adoption or insertion', 'NOT_READY', 'attempt status cannot open downstream gates')]

def build_entries() -> List[Entry]:
    return [
        Entry('P1: attempt target', 'separate symbolic paired trace-normalization declaration attempt', 'SYMBOLIC_PAIRED_DECLARATION_ATTEMPT', 'Group 50 may state the attempt record authorized by Group 49', 'not automatic success'),
        Entry('P2: paired domain', 'B_s_metric / b_s_scale paired branch-indexed domain', 'PAIRED_BRANCH_DOMAIN', 'the attempt remains paired and branch-indexed', 'not one neutral law'),
        Entry('P3: conditional status', 'attempt may survive only with explicit conditions', 'DECLARATION_ATTEMPT_CONDITIONAL_ONLY', 'symbolic d, numeric-d condition, caveats, and failure criteria remain attached', 'not full field-equation declaration'),
        Entry('P4: no downstream use', 'adoption, insertion, active O, residual/source safety, recombination, and parent closure remain outside the attempt', 'NOT_READY', 'the attempt cannot become field-equation machinery', 'not downstream opening'),
    ]

def build_shortcuts() -> List[Shortcut]:
    return [
        Shortcut('X1: attempt as adoption', 'treat declaration attempt as Package B adoption', 'adoption remains a separate theory decision'),
        Shortcut('X2: attempt as insertion', 'insert B_s/F_zeta because an attempt is stated', 'insertion remains not ready'),
        Shortcut('X3: attempt as branch choice', 'let the paired attempt choose metric or scale', 'branch choice requires separate explicit record'),
        Shortcut('X4: attempt as parent use', 'call the attempt parent-facing', 'parent identity and safety support are absent'),
    ]

def build_obligations() -> List[ObligationEntry]:
    return [
        ObligationEntry('O1: preserve attempt status', 'NOT_DECLARED', 'state the record as an attempt, not adopted declaration', 'avoid status inflation'),
        ObligationEntry('O2: preserve paired branch labels', 'POLICY_RULE', 'keep B_s_metric and b_s_scale explicit', 'avoid neutral-law collapse'),
