...
SCRIPT_LABEL = 'Candidate Branch Readiness Batch Reconciliation'
MARKER_ID = 'g39_recon'
DEPENDENCIES = [
...
def build_entries() -> List[Entry]:
    raw = [('Q1: route-readiness opener', 'Group 39 opened as branch-choice readiness audit', 'PASS', 'expected if no branch or route was chosen', 'summary may call this a readiness audit only'), ('Q2: route matrix', 'branch-required, split-safe, neutral-safe, conditional, and not-ready routes separated', 'PASS', 'expected if downstream routes remain closed', 'summary must preserve route distinctions'), ('Q3: split-safe continuations', 'some work can continue under split notation', 'PASS', 'expected if no active branch is installed', 'summary may allow split-safe audits'), ('Q4: neutral F_zeta boundary', 'neutral deferral is safe only without installed expression', 'PASS', 'expected if no hidden zeta/d or 2*zeta/d appears', 'summary must preserve neutral boundary'), ('Q5: blockers', 'blockers are open/deferred rather than branch no-go theorems', 'PASS', 'expected if branches remain live', 'summary must not kill either branch'), ('Q6: downstream gates', 'insertion, active O, residual control, and parent closure remain closed', 'NOT_READY', 'expected final boundary', 'summary must not open field-equation routes')]
...
def case_0(out):
    header(SCRIPT_LABEL)
    print('Question:\\n')
    for line in 'Did the Group 39 batch produce the expected branch-readiness audit shape, and what should a later status summary preserve?'.splitlines():
        print("  " + line if line else "")
    print('\\nDiscipline:\\n')
    print('This script reconciles the speculative Group 39 batch.')
    print('It does not close the group as a final summary.')
    print('It does not choose a branch, fill declarations, adopt, prove, insert, or open parent closure.')
    print('\\nTiny goblin rule:\\n  Count the locks after the tunnel run. Do not call it a treasure chest.')
...
def case_1(out):
    ...
    names = ['expected_problem', 'expected_matrix', 'expected_split', 'expected_neutral', 'expected_blockers', 'summary_ready', 'P_insertion', 'P_active_O', 'P_residual_kill', 'P_parent']
...
def case_2(out, entries):
    header('Case 2: Batch reconciliation entries')
    for item in entries:
        subheader(item.name)
...
def case_3(out):
    ...
    for name, shortcut, reason in [('X1: reconciliation as final summary', 'treat this script as group close', 'final summary should be written after actual outputs are reviewed'), ('X2: split-safe as branch choice', 'say safe continuation chose active branch', 'split-safe is not active declaration'), ('X3: blocker as theorem failure', 'say branch deferral proves no branch works', 'blockers are governance/evidence gaps'), ('X4: readiness as downstream readiness', 'use readiness audit to open insertion or parent route', 'downstream gates remain closed')]:
        subheader(name)
...
def case_4(out):
    ...
    for name, obligation, blocks, discipline in [('O1: summary must follow actual outputs', 'final summary must report any mismatch in batch outputs', 'speculative overclaim', 'actual outputs control close'), ('O2: preserve branch-deferred status', 'final summary must keep active branch unchosen unless actual script chose it', 'hidden choice', 'no choice by summary prose'), ('O3: preserve safe continuation classes', 'carry split-safe/neutral-safe/branch-required distinctions', 'route flattening', 'not every route has same branch need')]:
        subheader(name)
...
def case_5(out):
    ...
    with out.governance_assessments():
        for name, status, detail in [('batch reconciliation prepared', 'PASS', 'write summary only after actual outputs are reviewed'), ('no group close here', 'DEFER', 'candidate_group_39_status_summary.py should be written after review')]:
            out.line(name, mark(status), detail)
    print("\nPossible next script:")
    print(f"  candidate_group_39_status_summary.py")
...
def record_governance(ns, entries):
    ...
    for idx, item in enumerate([('O1: summary must follow actual outputs', 'final summary must report any mismatch in batch outputs', 'speculative overclaim', 'actual outputs control close'), ('O2: preserve branch-deferred status', 'final summary must keep active branch unchosen unless actual script chose it', 'hidden choice', 'no choice by summary prose'), ('O3: preserve safe continuation classes', 'carry split-safe/neutral-safe/branch-required distinctions', 'route flattening', 'not every route has same branch need')], 1):
