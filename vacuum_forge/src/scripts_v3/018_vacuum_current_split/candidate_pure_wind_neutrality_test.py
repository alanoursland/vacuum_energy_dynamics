...
def main():
    ...
    ns.record_obligation(ProofObligationRecord(
        ...
        script_id=SCRIPT_ID,
        title="Pure wind mass neutrality",
        status=ObligationStatus.OPEN,
        description="Mass neutrality theorem: delta M_ext|J_sub = 0. Pure wind must not change measured exterior mass.",
    ))
    ns.record_obligation(ProofObligationRecord(
        obligation_id="prove_pure_wind_scalar_trace_neutrality_in_18_pure_wind",
        script_id=SCRIPT_ID,
        title="Pure wind scalar trace neutrality",
        status=ObligationStatus.OPEN,
        description="Scalar trace neutrality theorem: J_sub must not source B_s, zeta residual metric trace, or scalar charge.",
    ))
    ns.record_obligation(ProofObligationRecord(
        obligation_id="prove_pure_wind_matter_decoupling_in_18_pure_wind",
        script_id=SCRIPT_ID,
        title="Pure wind matter decoupling",
        status=ObligationStatus.OPEN,
        description="Ordinary matter decoupling theorem: J_sub must not enter ordinary T_mu_nu routing or push matter.",
    ))
    ...
    ns.record_branch_decision(BranchDecisionRecord(
        ...
        script_id=SCRIPT_ID,
        branch_id="pure_wind_as_mass_source",
        status=GovernanceStatus.REJECTED_ROUTE,
        tier=ClaimTier.CONSTRAINED,
        description="Pure wind contributing directly to mass/energy source is forbidden under pure wind neutrality.",
    ))
    ns.record_branch_decision(BranchDecisionRecord(
        decision_id="reject_pure_wind_preferred_frame_force_in_18_pure_wind",
        script_id=SCRIPT_ID,
        branch_id="pure_wind_as_preferred_frame_force",
        status=GovernanceStatus.REJECTED_ROUTE,
        tier=ClaimTier.CONSTRAINED,
        description="Pure wind exerting ordinary force because it defines a preferred direction/frame is forbidden.",
    ))
    ns.record_branch_decision(BranchDecisionRecord(
        decision_id="reject_pure_wind_boundary_patch_in_18_pure_wind",
        script_id=SCRIPT_ID,
        branch_id="pure_wind_as_boundary_patch",
        status=GovernanceStatus.REJECTED_ROUTE,
        tier=ClaimTier.CONSTRAINED,
        description="Pure wind chosen to cancel boundary leakage or shell source is a forbidden boundary patch.",
    ))
    ns.record_derivation(
