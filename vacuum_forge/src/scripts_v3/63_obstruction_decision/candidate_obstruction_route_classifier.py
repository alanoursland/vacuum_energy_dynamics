...
SCOPE = 'Group 63 obstruction route classifier'
NEXT_SCRIPT = 'candidate_obstruction_batch_reconcile.py'

ENTRIES = [('K1: obstruction accepted', 'Group 62 obstruction controls status', 'OBSTRUCTION_INPUTS_RESTATED', 'stress accounting not closed', 'basis'), ('K2: insertion rejected', 'physical use now', 'INSERTION_REJECTED', 'not licensed', 'rejected'), ('K3: unqualified retention rejected', 'keep live candidate without contract', 'UNQUALIFIED_RETENTION_REJECTED', 'hides obstruction', 'rejected'), ('K4: diagnostic downgrade', 'preserve clues only', 'DIAGNOSTIC_DOWNGRADE_ALLOWED', 'clean allowed status', 'allowed'), ('K5: conditional audit retention', 'retain with explicit contract', 'CONDITIONAL_AUDIT_RETENTION_ALLOWED', 'allowed but burdened', 'conditional'), ('K6: escape requirements', 'valid future routes must derive stress principle/covariant lift/inertness', 'ESCAPE_REQUIREMENTS_DERIVED', 'not shortcuts', 'open'), ('K7: candidate status', 'not promoted', 'STRESS_ACCOUNTING_NOT_CLOSED', 'audit-only or diagnostic-only', 'not insertable'), ('K8: physical use', 'candidate remains blocked', 'PHYSICAL_USE_BLOCKED', 'no insertion, active O, recombination, or parent closure', 'not parent-ready')]
SHORTCUTS = [('X1: insert now', 'use response in field equation', 'rejected'), ('X2: keep as candidate silently', 'ignore obstruction contract', 'rejected'), ('X3: downgrade as deletion', 'lose useful diagnostics', 'diagnostic status preserves clues'), ('X4: shortcut escape', 'u/P/p_free/O by convenience', 'rejected')]
OBLIGATIONS = [('O1: explicit next route', 'DEFERRED_WITH_TARGET', 'next group should either attempt a real stress-origin route or record diagnostic-only downgrade', 'avoid looping'), ('O2: stress principle', 'ENERGY_ACCOUNTING_REQUIRED', 'derive stress-energy closure if conditional retention continues', 'avoid arbitrary stress'), ('O3: physical use block', 'NOT_INSERTABLE', 'keep insertion, active O, recombination, and parent closure closed', 'avoid status inflation')]
LOCAL_CONCLUSIONS = [('obstruction classified', 'PASS', 'candidate not promoted'), ('diagnostic downgrade allowed', 'DIAGNOSTIC_DOWNGRADE_ALLOWED', 'diagnostic-only status is cleanly available'), ('conditional retention allowed', 'CONDITIONAL_AUDIT_RETENTION_ALLOWED', 'retention only with explicit contract'), ('physical use remains blocked', 'PHYSICAL_USE_BLOCKED', 'no insertion or parent closure opened')]
