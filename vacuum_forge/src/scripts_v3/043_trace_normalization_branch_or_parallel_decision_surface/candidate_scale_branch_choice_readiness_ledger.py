...
CASE1_TITLE = 'Case 1: Scale branch readiness entries'
NEXT_SCRIPT = 'candidate_parallel_declaration_candidate_ledger.py'
CONCLUSIONS = [('scale branch ledger complete', 'PASS', 'b_s_scale support and obligations inventoried'), ('b_s_scale not chosen', 'DECLARATION_DEFERRED', 'scale branch remains future explicit-choice candidate only')]


def build_entries() -> List[Entry]:
    return [
        Entry('S1: scale candidate form', 'log(b_s_scale)=zeta/d may be carried as scale branch candidate form', 'BRANCH_CHOICE_CANDIDATE', 'candidate form remains visible for a later explicit choice record', 'not active declaration'),
        Entry('S2: scale-object clarity', 'scale choice would require a clear scale-factor object and scope', 'OPEN', 'b_s_scale must be typed before choice can support declaration', 'object clarity is not choice'),
        Entry('S3: zeta convention and dimension', 'scale choice requires explicit zeta convention, traced dimension d, and normalization scope', 'OPEN', 'scale form carries a different normalization burden than metric form', 'not supplied here'),
        Entry('S4: determinant/root intuition context', 'scale-factor intuition may be recorded as context only', 'ADMISSIBLE_CONTEXT', 'intuition may support a later explicit theory-owner choice', 'not derivation'),
        Entry('S5: recovery selector rejected', 'b_s_scale cannot be chosen from recovery, gamma, or parent fit', 'RECOVERY_SELECTOR_REJECTED', 'recovery is audit only', 'not an admissible selector'),
        Entry('S6: downstream insufficiency', 'scale branch choice alone would not open insertion or parent routes', 'NOT_READY', 'residual/source/boundary/divergence gates remain separate', 'branch choice is insufficient'),
    ]


def build_shortcuts() -> List[Shortcut]:
    return [
        Shortcut('X1: scale by determinant slogan', 'choose b_s_scale because determinant-root language feels cleaner', 'intuition requires explicit choice record, not hidden derivation'),
        Shortcut('X2: scale by recovery', 'choose b_s_scale because it fits AB=1, B=1/A, gamma, or Schwarzschild', 'recovery cannot select branch'),
        Shortcut('X3: scale as declaration', 'treat log(b_s_scale)=zeta/d as completed declaration', 'candidate form is not declaration'),
        Shortcut('X4: scale as insertion', 'insert b_s_scale/F_zeta after choosing the scale branch', 'insertion requires separate licensed law'),
    ]


def build_obligations() -> List[ObligationEntry]:
    return [
        ObligationEntry('O1: define scale branch object', 'OPEN', 'state b_s_scale domain, scope, and scale-factor meaning before any explicit choice', 'avoid branch ambiguity'),
        ObligationEntry('O2: state zeta and dimension conventions', 'OPEN', 'state zeta convention and traced dimension d before scale declaration', 'avoid hidden normalization'),
        ObligationEntry('O3: keep recovery selector rejected', 'POLICY_RULE', 'do not choose scale branch from recovery targets', 'construction must precede recovery audit'),
        ObligationEntry('O4: keep scale choice non-insertable', 'NOT_READY', 'do not treat scale branch choice as B_s/F_zeta insertion', 'insertion remains downstream'),
    ]
