...
SCOPE = 'Group 64 boundary variation sieve'
NEXT_SCRIPT = 'candidate_variational_route_classifier.py'

ENTRIES = [('B1: value silence', 'eta endpoints vanish', 'BOUNDARY_VARIATION_TESTED', 'value endpoint behavior is clean', 'reduced'), ('B2: slope silence', "eta' endpoints vanish", 'BOUNDARY_VARIATION_TESTED', 'slope endpoint behavior is clean', 'reduced'), ('B3: curvature burden', "eta'' endpoints nonzero", 'BOUNDARY_SILENCE_NOT_ORIGIN', 'second-derivative/bulk burden remains', 'open'), ('B4: origin status', 'boundary behavior alone', 'BOUNDARY_SILENCE_NOT_ORIGIN', 'endpoint silence does not derive the profile', 'not enough')]
SHORTCUTS = [('X1: boundary equals variational', 'claim endpoint silence proves origin', 'interior Euler equation also required'), ('X2: slope silence equals stress theorem', 'claim natural boundary terms derive stress tensor', 'bulk/closure missing')]
OBLIGATIONS = [('O1: full variational origin', 'ENERGY_ACCOUNTING_REQUIRED', 'derive both bulk Euler equation and boundary terms if candidate remains live', 'avoid boundary-only proof')]
LOCAL_CONCLUSIONS = [('boundary sieve applied', 'PASS', 'endpoint value/slope silence confirmed'), ('boundary not origin', 'BOUNDARY_SILENCE_NOT_ORIGIN', 'endpoint behavior alone cannot license candidate')]
...
def case_boundary_variation(out: ScriptOutput):
    header("Case 0: Boundary variation sieve")
    ...
    boundary_values = {
        "f(-1)": sp.simplify(f.subs(y, -1)),
        "f(1)": sp.simplify(f.subs(y, 1)),
        "fp(-1)": sp.simplify(fp.subs(y, -1)),
        "fp(1)": sp.simplify(fp.subs(y, 1)),
        "fpp(-1)": sp.simplify(fpp.subs(y, -1)),
        "fpp(1)": sp.simplify(fpp.subs(y, 1)),
    }
    ...
    with out.derived_results():
        for name, value in boundary_values.items():
            if "fpp" in name:
                status = StatusMark.OBLIGATION if not is_zero(value) else StatusMark.PASS
            else:
                status = StatusMark.PASS if is_zero(value) else StatusMark.FAIL
            out.line(name, status, f"{sp.factor(value)}")

    return {"fpp_left": boundary_values["fpp(-1)"], "fpp_right": boundary_values["fpp(1)"]}


def record_bdy(ns, data) -> None:
    ns.record_derivation(
        derivation_id="g64_bdy",
        inputs=[],
        output=sp.Symbol("boundary_silence_not_origin"),
        method="test endpoint value/slope/second-derivative behavior for variational boundary claims",
        status=Status.DERIVED,
        record_kind=RecordKind.DERIVATION,
        result_type="boundary_variation_sieve",
        scope="reduced boundary-variation audit; not variational theorem",
    )
...
def main() -> None:
    ...
    data = case_boundary_variation(out)
