...
The framework is organized into four numbered folders that preserve dependency order: foundations come first, vacuum exchange mechanism builds on them, the GR weak-field program builds on vacuum exchange, and the curvature exchange branch (Postulates 4 and 5) builds on the foundations in parallel.
...
The `process/` subfolder of this section contains documents that analyze the derivation problem for the Equal-Response component of the unity candidate, rather than framework content itself. They are work-on-the-framework rather than the framework's claims. Future readers do not need to read them to understand the theory; they exist as a record of how the foundational gap is currently understood and what a future derivation would have to accomplish.

- **[process] `process/candidate_paths_to_equal_response.md`** — classifies five candidate derivation paths for the Equal-Response component of unity ($\gamma_v = 1$): local Lorentz invariance, null propagation consistency, vacuum-as-one-substance, PPN reverse-engineering as constraint, and variational principle / energy minimization. Works through what each path forces and what it leaves open. Concludes that Equal-Response is not derivable from spherical symmetry alone, and identifies two plausible derivation routes — a structural route through the vacuum's mathematical structure (likely tensor-like) and a dynamical route through Postulate 5's energy minimization.

- **[process] `process/candidate_reciprocal_scale_equal_response.md`** — sharpens the Equal-Response target from "parallel scaling" (which gives the wrong metric) to "reciprocal scaling" $A(r) \cdot B(r) = 1$, where $A = \sqrt{-g_{00}}$ and $B = \sqrt{g_{ii}}$. Identifies $\mu(r) = \ln A + \ln B$ as the concrete mismatch variable Path 5 should minimize. Notes a structural connection: the same exponential ansatz $A = e^{\Phi/c^2}$, $B = e^{-\Phi/c^2}$ that gives $\gamma_v = 1$ at first order also matches the second-order time-metric candidate's $\beta = 1$ — the two provisional commitments may be two faces of one underlying reciprocal-scale principle. Honest about what's not established: the configuration energy functional that would dynamically force $\mu = 0$ has not been derived from existing postulates.
...
- **[done] `consequence_cosmic_structure_formation.md`** — works out what Postulate 5's mass-as-constraint reading and the cosmic expansion consequence together imply about cosmic-scale vacuum flow. The argument: cosmic expansion creates new vacuum everywhere (per the cosmic expansion consequence and Postulate 2); mass-pinned wells cannot freely absorb the new vacuum without violating their constraints; so the bulk of the new vacuum must migrate outward. Voids absorb the migrated vacuum because they are at unconstrained minimum. The cosmic web's qualitative geometry — voids surrounded by mass-rich filaments — emerges as the equilibrium pattern of this redistribution. Makes explicit one parsimonious assumption (expansion creates vacuum locally everywhere, including inside wells) and notes its framework-specific character relative to standard cosmology's bound-system decoupling. Includes the subtlety that wells contain less vacuum per coordinate span than flat regions and so create less new vacuum locally, which reinforces rather than complicates the redistribution picture. Sketches a continuity-equation form for vacuum extent as the natural quantitative target. Flagged explicitly as more speculative than the weak-field proofs — a high-level consequence with one parsimonious assumption rather than a derivation in the proof sense.

The following candidates are discussions or partial derivations associated with the curvature exchange branch. Each is an independent line of inquiry; they do not depend on one another, only on the parent postulates.
...
- **[done] `candidate_eternal_cosmic_renewal.md`** — sketches a long-term cosmological picture that the framework's commitments point toward: vast voids accumulate frothy vacuum at constant density, rare high-amplitude QFT fluctuations occasionally trigger runaway compression via Postulate 3's force-on-energy and Postulate 4's curvature-energy machinery, the runaway halts at the no-singularities limit producing a hot dense expanding bubble, and the resulting bubble ages into voids that may ignite further bubbles. Builds on the cosmic structure formation consequence and the no-singularities consequence, plus Postulates 3, 4, and 5. The central open mechanism is the threshold for runaway versus dissipation — under what conditions Postulate 5's relaxation drives compression rather than wave dissipation. Discusses observational signatures (the runaway threshold's prediction about initial conditions, possible CMB anomalies, subthreshold gravitational wave events) while honestly noting that the picture is closer to unfalsifiable than to testable without further framework development. Positions the picture relative to eternal inflation, conformal cyclic cosmology, and bubble-nucleation models. Flagged explicitly as more speculative than the cosmic structure formation consequence, which is itself more speculative than the weak-field proofs.
...
The four numbered folders (`01_foundations`, `02_vacuum_exchange`, `03_gr_weak_field`, `04_curvature_exchange`) preserve dependency order. Foundations contain the bedrock postulates and consequences. Vacuum exchange contains Postulate 3 and the basic mechanism derivations (redshift, time dilation). GR weak field is a topic-grouped folder containing the framework's program of reproducing classical weak-field GR tests. Curvature exchange contains Postulates 4 and 5 plus their consequences. The three numbered branch folders all build on the foundations layer; the GR weak field folder additionally builds on vacuum exchange.

The folder named `03_gr_weak_field` is topic-defined while the others are provenance-defined (each named for the postulate that heads its branch). This small inconsistency is deliberate: the GR-recovery program is a coherent body of work whose documents share methodology and depend on each other in ways that don't track the postulate hierarchy. Topic-based grouping captures what the program is.
...
For someone encountering this project for the first time, the documents should be read in the order below. Items are grouped by phase to make the framework's structure visible.
...
3. `postulate_vacuum_spacetime_identity.md`, for Postulate 1 (the ontological postulate: vacuum is spacetime is energy).
4. `postulate_vacuum_energy_density.md`, for Postulate 2 (the vacuum has finite locally constant energy density).
5. `consequence_curvature_as_spatial_differential.md`, for what curvature is under Postulates 1 and 2.
6. `consequence_cosmic_expansion.md`, for what Postulate 1 forces us to say about expansion.
7. `sr_mass_energy_equivalence.md`, for mass-energy equivalence (imported from SR, used heavily in derivations).

**Vacuum exchange mechanism.**

8. `postulate_vacuum_exchange_in_gradients.md`, for Postulate 3 (force and exchange in gradients).
9. `proof_gravitational_redshift.md`, for the first derivation from the core postulates.
10. `proof_gravitational_time_dilation.md`, for the second derivation, building on the redshift proof.

**GR weak-field program.**

11. `proof_weak_field_metric.md`, for the third derivation, consolidating the first two into partial weak-field metric form and naming the open spatial-response parameter $\gamma_v$.
12. `candidate_vacuum_variation_unity.md`, for the provisional closure of the weak-field metric at $\gamma_v = 1$ via the unity assumption.
13. `proof_light_deflection.md`, for the fourth derivation, which uses the closed weak-field metric to produce the deflection of light past a mass and makes the unity assumption's observational content explicit via the historical factor-of-two analysis.
14. `proof_shapiro_delay.md`, for the fifth derivation, which reproduces the Shapiro time delay formula through the same $(1+\gamma_v)$ machinery applied to temporal delay rather than angular deflection.
15. `proof_weak_field_equations_of_motion.md`, for the sixth derivation, which establishes how massive test bodies move in the weak-field regime through two routes (Postulate 3 directly and the geodesic equation), recovers Newton's law, and unlocks downstream proofs of bound-system effects.
...
The `process/` subfolder of `03_gr_weak_field/` is not part of the reading order. Process documents analyze the derivation problem for Equal-Response and are useful only to readers actively working on that problem; they are not required to understand the framework's current claims.
