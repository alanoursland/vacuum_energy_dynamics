...
The physical interpretation in the later section additionally invokes Postulate 1 and the curvature-as-spatial-volume-differential consequence. These are used to explain *why* the mapping between clocks at different potentials takes the form it does, not to derive the mapping itself. Readers who accept the formal derivation but are uncommitted on Postulate 1 can accept the quantitative result while reserving judgment on the interpretation.
...
The derivation depends only on Postulates 2 and 3, mass-energy equivalence, Postulate 1 (for interpretation), the curvature-as-spatial-volume-differential consequence (for interpretation), and special relativity. It does not invoke Postulates 4 or 5, which handle configuration energy and minimum-energy dynamics.
